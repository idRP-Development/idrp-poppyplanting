...
RegisterNetEvent('ps-poppyplanting:client:UsePoppySeed', function()
	...
	QBCore.Functions.Notify(_U('place_or_cancel'), 'success', 2500)
		...
		if hit == 1 then
			...
			if IsControlJustPressed(0, 38) then
				...
				QBCore.Functions.Progressbar('spawn_plant', _U('place_sapling'), 2000, false, true, {
					...
					TriggerServerEvent('ps-poppyplanting:server:CreateNewPlant', dest)
					planted = false
					seedUsed = false
					ClearPedTasks(ped)
...
RegisterNetEvent('ps-poppyplanting:client:CheckPlant', function(data)
	local netId = NetworkGetNetworkIdFromEntity(data.entity)
	QBCore.Functions.TriggerCallback('ps-poppyplanting:server:GetPlantData', function(result)
		...
		if result.health == 0 then -- Destroy plant
			exports['qb-menu']:openMenu({
				...
				},
				{
					...
					params = {
						event = 'ps-poppyplanting:client:ClearPlant',
						args = data.entity
					}
				}
			})
			...
			if PlayerJob.type == Shared.CopJob and PlayerJob.onduty then
				exports['qb-menu']:openMenu({
					...
					},
					{
						...
						params = {
							event = 'ps-poppyplanting:client:HarvestPlant',
							args = data.entity
						}
					},
					{
						...
						icon = 'fas fa-fire',
						params = {
							event = 'ps-poppyplanting:client:PoliceDestroy',
							args = data.entity
						}
					}
				})
			...
			if PlayerJob.type == Shared.CopJob and PlayerJob.onduty then
				...
				exports['qb-menu']:openMenu({
					...
					},
					{
						...
						icon = 'fas fa-shower',
						params = {
							event = 'ps-poppyplanting:client:GiveWater',
							args = data.entity
						}
					},
					{
						...
						params = {
							args = data.entity,
							event = 'ps-poppyplanting:client:GiveFertilizer',
						}
					},
				})
			end
...
RegisterNetEvent('ps-poppyplanting:client:ClearPlant', function(entity)
	...
	QBCore.Functions.Progressbar('clear_plant', _U('clear_plant'), 8500, false, true, {
		...
		TriggerServerEvent('ps-poppyplanting:server:ClearPlant', netId)
		ClearPedTasks(ped)
		RemoveAnimDict('amb@medic@standing@kneel@base')
		RemoveAnimDict('anim@gangops@facility@servers@bodysearch@')
	end, function()
...
RegisterNetEvent('ps-poppyplanting:client:HarvestPlant', function(entity)
	local netId = NetworkGetNetworkIdFromEntity(entity)
	local ped = PlayerPedId()
	TaskTurnPedToFaceEntity(ped, entity, 1.0)
	Wait(1500)
	...
	QBCore.Functions.Progressbar('harvest_plant', _U('harvesting_plant'), 8500, false, true, {
		...
		TriggerServerEvent('ps-poppyplanting:server:HarvestPlant', netId)
		ClearPedTasks(ped)
		RemoveAnimDict('amb@medic@standing@kneel@base')
		RemoveAnimDict('anim@gangops@facility@servers@bodysearch@')
	end, function()
...
RegisterNetEvent('ps-poppyplanting:client:PoliceDestroy', function(entity)
	local netId = NetworkGetNetworkIdFromEntity(entity)
	local ped = PlayerPedId()
	TaskTurnPedToFaceEntity(ped, entity, 1.0)
	Wait(500)
	ClearPedTasks(ped)
	TriggerServerEvent('ps-poppyplanting:server:PoliceDestroy', netId)
end)

RegisterNetEvent('ps-poppyplanting:client:FireGoBrrrrrrr', function(coords)
	local ped = PlayerPedId()
...
RegisterNetEvent('ps-poppyplanting:client:GiveWater', function(entity)
	if QBCore.Functions.HasItem(Shared.FullCanItem, 1) then
		...
		QBCore.Functions.Progressbar('poppyplanting_water', _U('watering_plant'), 6000, false, false, {
			disableMovement = true,
			disableCarMovement = true,
			disableMouse = false,
			disableCombat = true,
			...
			TriggerServerEvent('ps-poppyplanting:server:GiveWater', netId)
			ClearPedTasks(ped)
...
RegisterNetEvent('ps-poppyplanting:client:OpenFillWaterMenu', function()
	exports['qb-menu']:openMenu({
		{
		...
		},
		{
			...
			params = {
				event = 'ps-poppyplanting:client:FillWater',
			}
		}
	})
end)

RegisterNetEvent('ps-poppyplanting:client:FillWater', function()
	...
	QBCore.Functions.Progressbar('filling_water', _U('filling_water'), 2000, false, true, {
		...
		TriggerServerEvent('ps-poppyplanting:server:GetFullWateringCan')
	end, function() -- Cancel
		QBCore.Functions.Notify(_U('canceled'), 'error', 2500)
	end)
end)

RegisterNetEvent('ps-poppyplanting:client:GiveFertilizer', function(entity)
	if QBCore.Functions.HasItem(Shared.FertilizerItem, 1) then
		...
		QBCore.Functions.Progressbar('poppyplanting_fertilizer', _U('fertilizing_plant'), 6000, false, false, {
			disableMovement = true,
			disableCarMovement = true,
			disableMouse = false,
			disableCombat = true,
			...
			TriggerServerEvent('ps-poppyplanting:server:GiveFertilizer', netId)
			ClearPedTasks(ped)
			DeleteEntity(created_object)
		end, function()
			ClearPedTasks(ped)
...
CreateThread(function()
	exports['qb-target']:AddTargetModel(Shared.PoppyProps, {
		options = {
			{
				type = 'client',
				event = 'ps-poppyplanting:client:CheckPlant',
